...
export const useCadastroIntegrantes = () => {
  ...
  const {
    mutateAsync: cadastrarIntegrantes,
    isPending: isPendingCadastrarIntegrantes,
  } = useCadastrarIntegrantes(idGrupoNumber);

  const emails = watch("emailIntegrante") || [];

  const onSubmit = (data: FormData) => {
    const emailsParaEnviar = data.emailIntegrante.filter(
      (_, index) => !camposDesabilitados[index]
    );
      ...
      { email: emailsParaEnviar },
      {
        onSuccess: () => {
          ...
          setCamposDesabilitados((prev) =>
            prev.map((_, index) =>
              emailsParaEnviar.includes(data.emailIntegrante[index])
                ? true
                : prev[index]
            )
          );
        ...
        onError: (err: any) => {
          const message =
            err?.response?.data?.message || "Erro ao cadastrar integrante.";
          toast.error(message);
        },
      }
    );
